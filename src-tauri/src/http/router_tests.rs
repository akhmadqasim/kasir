...
/// `status` needs a session but no particular role — any cashier can see
/// whether the feature is connected.
#[tokio::test]
async fn the_whatsapp_status_route_needs_a_session_and_reports_a_shape_with_no_qr_by_default() {
    let db = setup_test_db().await;
    crate::test_support::insert_store_info(&db, false).await;
    let token = login_token(&db, 1).await;
    let state = state(db);
    ...
    let anonymous = router(&state)
        .oneshot(
            same_origin(Method::GET, "/api/whatsapp/status")
                .body(Body::empty())
                .expect("request"),
        )
        .await
    ...
    assert_eq!(body["state"], json!("off"));
    assert!(
        body.get("qr").is_none(),
        "a fresh manager has no QR to hide"
    );
}

/// `enable`/`disable`/`logout` are admin actions, and closed even to an admin
/// unless the request came in over loopback — the same rule the till window's
/// zoom and icon use.
#[tokio::test]
async fn enable_disable_and_logout_need_an_admin_session_from_the_till_itself() {
    let db = setup_test_db().await;
    crate::test_support::insert_store_info(&db, false).await;
    let kasir = insert_user_with_pin(&db, "kasir1", "1234", "kasir").await;
    let admin = insert_user_with_pin(&db, "admin2", "1234", "admin").await;
    ...
    for path in [
        ...
        let as_kasir = router(&state)
            .oneshot(
                from_address(same_origin(Method::POST, path), [127, 0, 0, 1])
                    .header(header::COOKIE, cookie(&kasir_token))
                    .body(Body::empty())
                    .expect("request"),
            )
            .await
            .expect("response");
        assert_eq!(as_kasir.status(), StatusCode::FORBIDDEN, "{path}");

        // An admin, but from the shop LAN rather than the till itself: refused
        // before the sidecar is ever touched, so this needs no spawn factory.
        let from_lan = router(&state)
            .oneshot(
                from_address(same_origin(Method::POST, path), [192, 168, 1, 20])
                    .header(header::COOKIE, cookie(&admin_token))
                    .body(Body::empty())
                    .expect("request"),
            )
            .await
            .expect("response");
        assert_eq!(from_lan.status(), StatusCode::CONFLICT, "{path}");
        assert_eq!(body_json(from_lan).await["code"], json!("conflict"));
    }
}

/// `send-receipt` is open to any cashier session — no loopback rule, since a
/// tablet at the till counter sends receipts too.
#[tokio::test]
async fn send_receipt_needs_only_a_session_not_the_till_or_an_admin() {
    let db = setup_test_db().await;
    crate::test_support::insert_store_info(&db, false).await;
    let kasir = insert_user_with_pin(&db, "kasir1", "1234", "kasir").await;
    let token = login_token(&db, kasir.id).await;
    ...
    // Not enabled/ready and the transaction does not exist either, so this is
    // refused — the point here is only that a cashier session reaches the
    // handler at all, from anywhere.
    let refused = router(&state)
        .oneshot(
            from_address(
                same_origin(Method::POST, "/api/whatsapp/send-receipt"),
...
/// printer footer text.
#[tokio::test]
async fn the_whatsapp_settings_write_is_closed_to_a_cashier() {
    let db = setup_test_db().await;
    crate::test_support::insert_store_info(&db, false).await;
    let kasir = insert_user_with_pin(&db, "kasir1", "1234", "kasir").await;
    let token = login_token(&db, kasir.id).await;
    ...
    let response = router(&state)
        .oneshot(
            same_origin(Method::PUT, "/api/whatsapp/settings")
                .header(header::COOKIE, cookie(&token))
                .header(header::CONTENT_TYPE, "application/json")
                .body(json_body(json!({ "caption_template": "Halo {store}" })))
                .expect("request"),
        )
        .await
        .expect("response");
...
/// The read (`GET /whatsapp/settings`) is open to any session — the send
/// dialog needs the caption preview and is not an admin-only screen.
#[tokio::test]
async fn the_whatsapp_settings_read_is_open_to_a_cashier_and_defaults_sensibly() {
    let db = setup_test_db().await;
    crate::test_support::insert_store_info(&db, false).await;
    let kasir = insert_user_with_pin(&db, "kasir1", "1234", "kasir").await;
    let token = login_token(&db, kasir.id).await;
    ...
    let response = router(&state)
        .oneshot(
            same_origin(Method::GET, "/api/whatsapp/settings")
                .header(header::COOKIE, cookie(&token))
                .body(Body::empty())
                .expect("request"),
        )
        ...
        .expect("response");
    assert_eq!(response.status(), StatusCode::OK);
    assert!(body_json(response).await["caption_template"]
        .as_str()
        .is_some_and(|text| text.contains("{store}")));
}
