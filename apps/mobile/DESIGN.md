...
| **Daftar panjang**         | `List` SwiftUI asli bergaya `insetGrouped` — inset, hairline menjorok ke teks, pull-to-refresh, chevron sistem, rubber-banding                                                   | `FlatList` baris M3 dengan pemisah                                                    | `components/native-list.ios.tsx` memakai `@expo/ui/swift-ui` (`Host`/`List`/`Section`), dipilih lewat `hasSwiftUI`; `components/native-list.tsx` adalah stub supaya SwiftUI tidak ikut ke bundle Android. Fallback-nya `FlatList` yang sudah ada — paging (`onAppear` baris terakhir vs `onEndReached`) dan refresh sama di dua jalur  |
| **Grup kecil**             | Inset grouped: judul bagian kecil di luar grup, hairline antar baris                                                                                                             | Baris M3 di atas `surface`, judul bagian beraksen, tanpa pemisah                      | `Section` (`components/section.tsx`), satu-satunya pemilik aturan pemisah. `variant`: `facts` (label/nilai), `fields` (kolom isian), `rows` (`ListGroup.Item`). Jangan susun grup sendiri dari `ListGroup`                                                                                                                             |
...
| **Kamera**                 | Tombol lebar penuh berlabel di bawah pratinjau, SF Symbol `camera.fill`/`camera`. Bukan ikon melayang: tab tidak punya navigation bar, dan toolbar melayang akan menimpa tab bar | Sama, ikon `MaterialCommunityIcons`                                                   | `(tabs)/scan.tsx` + `scanner-store.ts`. Kamera mati → kartu selebar pratinjau menjelaskan alasannya; tombolnya tetap di bawah kartu                                                                                                                                                                                                    |
| **Garis pindai**           | Garis tipis beraksen menyapu naik-turun selama kamera hidup                                                                                                                      | sama                                                                                  | `components/scan-frame.tsx` — `Animated` inti RN, `useNativeDriver`, berhenti saat kamera mati. Reduce Motion → garis diam di tengah                                                                                                                                                                                                   |
...
- `@expo/ui/swift-ui` dan `expo-symbols` adalah dependensi `expo-router`, jadi keduanya
  ikut terpasang di Expo Go. `@expo/ui` menyelesaikan native view-nya **saat impor**,
  bukan saat render, sehingga modul SwiftUI hanya boleh diimpor dari berkas `.ios.tsx`;
  `hasSwiftUI` (`lib/native-modules.ts`) yang memilih jalurnya saat berjalan.
