...
export function usePopularProducts(startDate: string, endDate: string, limit: number) {
  const range = { startDate, endDate }
  return useApiQuery<PopularProductRow[]>(queryKeys.reports.popularProducts(range, limit), () =>
...
export function useCurrentStock(search: string, filter: "all" | "low") {
  return useApiQuery<CurrentStockReport>(queryKeys.reports.currentStock(search, filter), () =>
